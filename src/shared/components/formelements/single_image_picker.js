...
const ImagePicker=(props)=>{
	...
	const pickedHandler=(event)=>{
		let pickedFile;
		let fileValid=valid;
		if(event.target.files && event.target.files.length===1 && event.target.files[0].size*0.000001<=3){
			...
			setValid(false);
			fileValid=false;
			if(event.target.files && event.target.files[0].size*0.000001>3){
